...
class BaseExecutionTimePredictor(ABC):
    ...
    def get_execution_time(self, batch: Batch, pipeline_stage: int) -> ExecutionTime:
        ...
        if self._config.backend == "aicb":
            ...
            has_prefill = batch.num_prefill_tokens > 0
            has_decode = batch.num_decode_tokens > 0

            if has_prefill and has_decode:
                raise NotImplementedError(
                    "AICB backend does not support mixed prefill/decode batches"
                )
            elif has_prefill:
                ...
                raise ValueError("AICB backend received an empty batch")
            ...
            assert len(batch.requests) == 1, "corrected profiles require one request"
            request = batch.requests[0]
            replica_config.profile_prompt_tokens = request.num_prefill_tokens
            replica_config.profile_kv_tokens = request.num_processed_prefill_tokens + batch.num_tokens[0]
